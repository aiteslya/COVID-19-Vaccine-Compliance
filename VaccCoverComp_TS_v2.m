%This is the script that percentage if vaccinated individuals against
%COVID-19 individuals (with one or two doses) in four countries: Belarus,
%Israel, the Netherlands, and Portugal for the first 6 months of the 
% vaccinaiton rollout programme. It then calculates the vaccination rate
% corresponding to the data and outputs the rate with which compliance
% fades given a constant incidence of 16, 062 cases per day
...
TblBEL = readtable('DataVaccine3/Belarus.csv');
TblISR = readtable('DataVaccine3/Israel.csv');
TblNED = readtable('DataVaccine3/Netherlands.csv');
TblPOR = readtable('DataVaccine3/Portugal.csv');
...
dates=TblPOR.date;
DATEPOR=datenum(dates)-datenum(dates(1))+1;

dates=TblNED.date;
DATENED=datenum(dates)-datenum(dates(1))+1;

TBLBEL=[DATEBEL';(TblBEL.people_vaccinated_per_hundred)']';
TBLBEL(:,1)=TBLBEL(:,1)-1;

TBLISR=[DATEISR';(TblISR.people_vaccinated_per_hundred)']';
TBLISR(:,1)=TBLISR(:,1)-1;

TBLNED=[DATENED';(TblNED.people_vaccinated_per_hundred)']';
TBLNED(:,1)=TBLNED(:,1)-1;

TBLPOR=[DATEPOR';(TblPOR.people_vaccinated_per_hundred)']';
TBLPOR(:,1)=TBLPOR(:,1)-1;
...
h2(3)=plot(TBLBEL(:,1),TBLBEL(:,2),'*','MarkerSize',6,'color',col_p(1,:));hold on;
h2(4)=plot(TBLISR(:,1),TBLISR(:,2),'*','MarkerSize',6,'color',col_p(2,:));hold on;
h2(5)=plot(TBLNED(:,1),TBLNED(:,2),'^','MarkerSize',6,'color',[0.5,0.5,0.5]);hold on;
h2(6)=plot(TBLPOR(:,1),TBLPOR(:,2),'o','MarkerSize',6,'color',[0.5,0.5,0.5]);hold on;
...
legend(h2,'Slow vaccination','Fast vaccination','Belarus data', 'Israel data','Netherlands data','Portugal data','Location','southoutside','NumColumns',2);
set(gca,'FontSize',25);
