...
% two vaccination uptake rates and two vaccines efficacies with respect to sterilization are considered
...
hosp_prob_arr(1,1)=0.04;%0.096;%probability of becoming hospitalized when infected with original strain and non-vaccinated
...
for epsilon=epsilonarr(1:2)
    ...
    for upsilon0=upsilonarr
        if upsilon0==0
            if epsilon==epsilonarr(1)
                hosp_prob_vacc=hosp_prob_arr(1,1);
            else
                hosp_prob_vacc=hosp_prob_arr(1,2);
            end
        else
            if epsilon==epsilonarr(1)
        ...
        %choose hospitalization probability

        infectious=(y(:,2)+y(:,3)+y(:,6)+y(:,7)+y(:,11)+y(:,12)+y(:,15)+y(:,16))/XpopOut; % time series
        hosp=(hosp_prob_unvacc*(y(:,3)+y(:,7)+y(:,12))+hosp_prob_vacc*(y(:,16)))/XpopOut; % timer series
        cum=(y(:,2)+y(:,3)+y(:,4)+y(:,6)+y(:,7)+y(:,8)+y(:,11)+y(:,12)+y(:,13)+y(:,15)+y(:,16)+y(:,17))/XpopOut-infect0pop;
        %new
        cumvacc=(y(:,15)+y(:,16)+y(:,17))/XpopOut; % only these who were infected after the vaccination event, not these who were vaccinated while infected
        compl=(y(:,5)+y(:,6)+y(:,7)+y(:,8))/XpopOut;

        %cumulative percentage of new hospitalizations since the start of the vaccination
        ...
        cum_hosp_vacc=100*(hosp_prob_vacc*(y(:,16)-y(1,16)+y(:,17)-y(1,17)))/N;
        if upsilon0==0 %plot 
            %%%
            if i1==1
                figure(1);
                subplot(1,3,1);
                h1(1)=plot(t,infectious,'-.','color',novacc,'LineWidth',4);hold on;
                % hospitalized
                figure(2);
                subplot(1,3,1);
                h2(1)=plot(t,hosp,'-.','color',novacc,'LineWidth',4);hold on;
            else
                figure(1);
                subplot(1,3,2);
                ...
                figure(2);
                subplot(1,3,2);
                plot(t,hosp,'-.','color',novacc,'LineWidth',4);hold on;
            ...
            %baseCum(i1,1)=100*cum_hosp(ind1year)/N;
            baseCum(i1,1)=cum_hosp(ind1year);
            %find index of the second year
            ind2year=find(t>180,1);
            %baseCum(i1,2)=100*cum_hosp(ind2year)/N;
            baseCum(i1,2)=cum_hosp(ind2year);
            disp('Baseline cumulative infections have been collected');
        elseif upsilon0==upsilonarr(2)
            if i1==1 %variant type
                figure(1);
                subplot(1,3,1);
                h1(2)=plot(t,infectious,'LineWidth',4,'color',col_p(1,:));hold on;
                % hospitalized
                figure(2);
                subplot(1,3,1);
                h2(2)=plot(t,hosp,'color',col_p(1,:),'LineWidth',4);hold on;
                ...
                figure(11);% v1, slow
                subplot(1,3,1);
                h11(2)=area(t,100*cum_hosp/N,'facecolor',novacc,'edgecolor','none');hold on;
                h11(3)=area(t,cum_hosp_vacc,'facecolor',medblue,'edgecolor','none');hold on;
                xlim([0,T]);
                ylim([0,2]);
            else
                figure(1);
                ...
                figure(2);
                subplot(1,3,2);
                plot(t,hosp,'color',col_p(1,:),'LineWidth',4);hold on;
                ...
                figure(11);% v2, slow
                subplot(1,3,2);
                h11(4)=area(t,100*cum_hosp/N,'facecolor',novacc,'edgecolor','none');hold on;
                h11(5)=area(t,cum_hosp_vacc,'facecolor',medblue,'edgecolor','none');hold on;
                xlim([0,T]);
                ylim([0,2]);
            end
        else
            if i1==1
                figure(1);
                subplot(1,3,1);
                h1(3)=plot(t,infectious,'LineWidth',4,'color',col_p(2,:));hold on;
                % hospitalized
                figure(2);
                subplot(1,3,1);
                h2(3)=plot(t,hosp,'color',col_p(2,:),'LineWidth',4);hold on;
               ...
               figure(12);% v1, fast
               subplot(1,3,1);
               h12(2)=area(t,100*cum_hosp/N,'facecolor',novacc,'edgecolor','none');hold on;
               h12(3)=area(t,cum_hosp_vacc,'facecolor',medblue,'edgecolor','none');hold on;
               xlim([0,T]);
               ylim([0,2]);
            else
                figure(1);
                ...
                figure(2);
                subplot(1,3,2);
                plot(t,hosp,'color',col_p(2,:),'LineWidth',4);hold on;
                ...
                figure(12);% v2, fast
                subplot(1,3,2);
                h12(4)=area(t,100*cum_hosp/N,'facecolor',novacc,'edgecolor','none');hold on;
                h12(5)=area(t,cum_hosp_vacc,'facecolor',medblue,'edgecolor','none');hold on;
                xlim([0,T]);
                ylim([0,2]);
            end
        end
        if upsilon0>0
            ind1year=find(t>90,1);
            CumContainer(2*i1-2+i2,1)=cum_hosp(ind1year);
            ind2year=find(t>180,1);
            CumContainer(2*i1-2+i2,2)=cum_hosp(ind2year);
            i2=i2+1;
        end
...
xlabel('Time (days)','interpreter','latex');
ylabel('Hospitalized individuals (1/100,000)','interpreter','latex');
title('Original variant');
%legend('Baseline, no vaccination','Slow vaccination, low efficacy','Fast vaccination, low efficacy','location','southoutside','NumColumns',3);
set(gca,'FontSize',25);
...
title('Original variant');
ylabel('Excess hospitalizations (\%)','interpreter','latex');
%ylim([-5,70]);
...
xlabel('Time (days)','interpreter','latex');
%ylabel({'Cumulative number of infected';'individuals (1/100,000)'},'interpreter','latex');
ylabel('Cumulative percentage hospitalized ($$\%$$)','interpreter','latex');
title('Original variant','FontWeight','normal');
%legend(h8,'Baseline, no vaccination    ','Total, with vaccination     ','Vaccinated, with vaccination','Total, with vaccination     ','Vaccinated, with vaccination','NumColumns',2,'location','southoutside');
...
xline(90,'-.','color',yearcol,'LineWidth',2);
xline(180,'-.','color',yearcol,'LineWidth',2);
legend(h11(4:5),'Non-vaccinated','Vaccinated','NumColumns',2,'location','southoutside');
%ylim([0,30]);
%ylim([0,100]);
xlabel('Time (days)','interpreter','latex');
title('Alpha-like variant','FontWeight','normal');
...
for upsilon0=upsilonarr
    if upsilon0==0
        hosp_prob_vacc=hosp_prob_arr(1,3);
    else
    ...
    cum=(y(:,2)+y(:,3)+y(:,4)+y(:,6)+y(:,7)+y(:,8)+y(:,11)+y(:,12)+y(:,13)+y(:,15)+y(:,16)+y(:,17))/XpopOut-infect0pop;
    %hospitalized
    hosp=(hosp_prob_unvacc*(y(:,3)+y(:,7)+y(:,12))+hosp_prob_vacc*(y(:,16)))/XpopOut;
    %new
...
%         h8(3)=area(t,100*cumvacc/popOut,'facecolor',medblue,'edgecolor','none');hold on;
        ...
        figure(11);% v1, slow
        subplot(1,3,3);
        area(t,100*cum_hosp/N,'facecolor',novacc,'edgecolor','none');hold on;
        area(t,cum_hosp_vacc,'facecolor',medblue,'edgecolor','none');hold on;
        xlim([0,T]);
        ylim([0,2]);
    else
        figure(1);
    ...
    for upsilon0=upsilonarr(2:3)
        ...
        if upsilon0==upsilonarr(2)
            if i1==1
                figure(1);
                subplot(1,3,1);
                h1(4)=plot(t,infectious,'-','LineWidth',4,'color',col_p2(1,:));hold on;
                % hospitalized
                figure(2);
                subplot(1,3,1);
                h2(4)=plot(t,hosp,'-','color',col_p2(1,:),'LineWidth',4);hold on;
            else
                figure(1);
                subplot(1,3,2);
                ...
                figure(2 );
                subplot(1,3,2);
                plot(t,hosp,'-','LineWidth',4,'color',col_p2(1,:));hold on;
            end
        else
            if i1==1
                figure(1);
                subplot(1,3,1);
                h1(5)=plot(t,infectious,'-','LineWidth',4,'color',col_p2(2,:));hold on;
                %hospitalized
                figure(2);
                subplot(1,3,1);
                h2(5)=plot(t,hosp,'-','LineWidth',4,'color',col_p2(2,:));hold on;
            else
                figure(1);
                subplot(1,3,2);
                ...
                figure(2);
                subplot(1,3,2);
                plot(t,hosp,'-','LineWidth',4,'color',col_p2(2,:));hold on;
            end
        end
        if upsilon0>0
            ind1year=find(t>=90,1);
            CumContainer(2*i1+i2-2,1)=cum_hosp(ind1year);
            ind2year=find(t>=180,1);
            CumContainer(2*i1+i2-2,2)=cum_hosp(ind2year);
            i2=i2+1;
        end
...
ylabel('Infected individuals (1/100,000)','interpreter','latex');
title('Original variant');
legend(h1,'Baseline, no vaccination','Slow vaccination, low efficacy','Fast vaccination, low efficacy','Slow vaccination, high efficacy','Fast vaccination, high efficacy','location','southoutside','NumColumns',3);
set(gca,'FontSize',25);
...
ylabel('Hospitalized individuals (1/100,000)','interpreter','latex');
title('Original variant');
legend(h2,'Baseline, no vaccination','Slow vaccination, low efficacy','Fast vaccination, low efficacy','Slow vaccination, high efficacy','Fast vaccination, high efficacy','location','southoutside','NumColumns',3);
set(gca,'FontSize',25);
...
figure(1);annotation('textbox', [0.05, 0.99, 0, 0], 'string', 'a','FontWeight','bold','FontSize',30)
figure(1);annotation('textbox', [0.35, 0.99, 0, 0], 'string', 'b','FontWeight','bold','FontSize',30)
figure(1);annotation('textbox', [0.66, 0.99, 0, 0], 'string', 'c','FontWeight','bold','FontSize',30)

figure(2);annotation('textbox', [0.05, 0.99, 0, 0], 'string', 'c','FontWeight','bold','FontSize',30)
figure(2);annotation('textbox', [0.35, 0.99, 0, 0], 'string', 'd','FontWeight','bold','FontSize',30)
figure(2);annotation('textbox', [0.66, 0.99, 0, 0], 'string', 'e','FontWeight','bold','FontSize',30)
...
figure(11);annotation('textbox', [0.05, 0.96, 0, 0], 'string', 'a','FontWeight','bold','FontSize',30)
figure(11);annotation('textbox', [0.35, 0.96, 0, 0], 'string', 'b','FontWeight','bold','FontSize',30)
figure(11);annotation('textbox', [0.66, 0.96, 0, 0], 'string', 'c','FontWeight','bold','FontSize',30)

figure(12);annotation('textbox', [0.05, 0.96, 0, 0], 'string', 'd','FontWeight','bold','FontSize',30)
figure(12);annotation('textbox', [0.35, 0.96, 0, 0], 'string', 'e','FontWeight','bold','FontSize',30)
figure(12);annotation('textbox', [0.66, 0.96, 0, 0], 'string', 'f','FontWeight','bold','FontSize',30)