...
% we investigate effects of variation in contact rates of compliant and 
% non-compliant non-vaccinated indvidualswith variation
...
carr=linspace(5,chat,meshsize);
r1arr=linspace(0.01,1,meshsize);

[Carr,R1arr]=meshgrid(carr,r1arr);
...
for upsilon=upsilonarr
    ...
    for i1=1:meshsize
        for i2=1:meshsize 
            ...
            %collect outputs without vaccination
            upsilontemp=0;
    ...
    s=surf(Carr,R1arr,Cum3);
    view(2);
    ylim([0.01,1]);
    ylabel({'Ratio between contact rates';'of compliant and non-compliant'},'interpreter','latex');
    colormap jet

    s.EdgeColor = 'none';
    hold on;
    
    [M,C]=contour(Carr,R1arr,Cum3,[0,0]);
    C.LineColor='m';
    C.LineWidth=4;
    xlabel({'Contact rate of';'non-compliant individuals'},'interpreter','latex');
    ...
    %relative difference in the cumulative number of infections
    f2=figure(2);
    subplot(2,2,subfigc);
    
    s=surf(Carr,R1arr,100*(Cum3-Cum03)./Cum03);
    view(2);
    ylim([0.01,1]);
    ylabel({'Ratio between contact rates';'of compliant and non-compliant'},'interpreter','latex');
    colormap jet
    ...
    [M,C]=contour(Carr,R1arr,100*(Cum3-Cum03)./Cum03,[0,0]);
    C.LineColor='m';
    C.LineWidth=4;
    xlabel({'Contact rate of';'non-compliant individuals'},'interpreter','latex');
    ...
    f1=figure(2);
    subplot(2,2,2+subfigc);
    s=surf(Carr,R1arr,100*(Cum6-Cum06)./Cum06);hold on;
    s.EdgeColor = 'none';
    view(2)
    ylim([0.01,1]);
    xlim([5,15]);
    ...
    [M,C]=contour(Carr,R1arr,100*(Cum6-Cum06)./Cum06,[0,0]);
    C.LineColor='m';
    C.LineWidth=4;
    xlabel({'Contact rate of';'non-compliant individuals'},'interpreter','latex');
