...
% of recovered individuals, the rest of the epidemiological compartments
...
% vaccination rollout
% the outputs are the cumulative number of infected at the mark of 3 and 6
% months from the start of the vaccination rollout
...
% define recovered proportions
RecovProp=linspace(0.05,0.2,meshsize);

%calculate the values without vaccination
...
for upsilon=upsilonarr
    ...
    h1(1)=plot(100*RecovProp,Cum3,'LineWidth',4);hold on;
    h1(2)=plot(100*RecovProp,Cum6,'-.','LineWidth',4);hold on;
    xlim([min(100*RecovProp),max(100*RecovProp)]);
    ylim([1,3.5]);
    xlabel({'Percentage of recovered individuals';'at the start of the vaccination rollout (\%)'},'interpreter','latex');
...
%     end
    ...
    h2(1)=plot(100*RecovProp,100*(Cum3-Cum03)./Cum03,'LineWidth',4);hold on;
    h2(2)=plot(100*RecovProp,100*(Cum6-Cum06)./Cum06,'-.','LineWidth',4);hold on;
    xlim([min(100*RecovProp),max(100*RecovProp)]);
    %ylim([1,3.5]);
    xlabel({'Percentage of recovered individuals';'at the start of the vaccination rollout (\%)'},'interpreter','latex');
...
%annotations
figure(2);annotation('textbox', [0.08, 0.99, 0, 0], 'string', 'c','FontWeight','bold','FontSize',25)
figure(2);annotation('textbox', [0.53, 0.99, 0, 0], 'string', 'd','FontWeight','bold','FontSize',25)
