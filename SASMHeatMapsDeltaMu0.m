...
% coverage grows and significant foregoing of compliance,
% we investigate effects of variation in rates of gaining and losing
% compliance (in the mode of zero vaccination coverage)
...
deltaLogarr=linspace(-6,-4,meshsize);
mu0arr=linspace(1/30,1/7,meshsize);

[DeltaLogarr,Mu0arr]=meshgrid(deltaLogarr,mu0arr);
...
for upsilon=upsilonarr
    ...
    for i1=1:meshsize
        for i2=1:meshsize 
            delta=10.^DeltaLogarr(i1,i2);
            mu0=Mu0arr(i1,i2);
            %simulate no-vaccination scenario for comparison
            upsilon0=0;
    ...
    s=surf(DeltaLogarr,1./Mu0arr,Cum3);
    ylim([7,30]);
    colormap jet
    
    xlabel({'$$\log$$ Per capita rate of moving to';'compliant mode ($$\log$$ 1/individual)'},'interpreter','latex');
    ylabel({'Duration of compliant mode when';'there is no vaccination (days)'},'interpreter','latex');
    ...
    f2=figure(1);
    subplot(2,2,subfigc+2);
    s=surf(DeltaLogarr,1./Mu0arr,Cum6);
    ylim([7,30]);
    colormap jet
    if subfigc==2
        colorbar;
    ...
    s=surf(DeltaLogarr,1./Mu0arr,100*(Cum3-Cum03)./Cum03);
    ylim([7,30]);
    colormap jet
    ...
    [M,C]=contour(DeltaLogarr,1./Mu0arr,100*(Cum3-Cum03)./Cum03,[0,0]);
    C.LineColor='m';
    C.LineWidth=4;
    caxis([-30,40])
    if upsilon==upsilonarr(1) 
    ...
    hold on;
    view(2);
    [M,C]=contour(DeltaLogarr,1./Mu0arr,100*(Cum6-Cum06)./Cum06,[0,0]);
    C.LineColor='m';
    C.LineWidth=4;
    caxis([-30,40])
...
%annotations
figure(1);annotation('textbox', [0.08, 0.99, 0, 0], 'string', 'a','FontWeight','bold','FontSize',20)
figure(1);annotation('textbox', [0.53, 0.99, 0, 0], 'string', 'b','FontWeight','bold','FontSize',20)

figure(1);annotation('textbox', [0.08, 0.49, 0, 0], 'string', 'c','FontWeight','bold','FontSize',20)
figure(1);annotation('textbox', [0.53, 0.49, 0, 0], 'string', 'd','FontWeight','bold','FontSize',20)
