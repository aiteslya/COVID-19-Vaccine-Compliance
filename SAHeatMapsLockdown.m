...
% number of new infections relative to no vaccination scenario without lockdown
...
for pc=thresharr
    ...
    if subfigc==2
     ...
     if pc==thresharr(1)
         ylabel({'\textbf{Lockfown threshold 50 individuals}';' ';'Vaccine efficacy (\%)'},'interpreter','latex');
     else
         ylabel({'\textbf{Lockfown threshold 500 individuals}';' ';'Vaccine efficacy (\%)'},'interpreter','latex');
     end
     if subfigc==1
    ...
    if subfigc==1
        title('Six months','interpreter','latex','FontSize',23);
    end
    set(gca,'FontSize',12);
