...
% to the variations of initial data with respect to compliance
...
%output the pairs of contact rates
figure(20);
...
ComplProp=linspace(0.2,0.9,meshsize);

% calculate the results wihout vaccination
...
for upsilon=upsilonarr
    ...
    h1(1)=plot(100*ComplProp,Cum3,'LineWidth',4);hold on;
    h1(2)=plot(100*ComplProp,Cum6,'-.','LineWidth',4);hold on;
    xlim([min(100*ComplProp),max(100*ComplProp)]);
    ylim([1.5,3]);
    xlabel({'Percentage of compliant individuals';'at the start of the vaccination rollout (\%)'},'interpreter','latex');
    ...
    %output relative to no-vaccination scenario
    
    f2=figure(2);
    subplot(1,2,subfigc);
    
    h2(1)=plot(100*ComplProp,100*(Cum3-Cum03)./Cum03,'LineWidth',4);hold on;
    h2(2)=plot(100*ComplProp,100*(Cum6-Cum06)./Cum06,'-.','LineWidth',4);hold on;
    xlim([min(100*ComplProp),max(100*ComplProp)]);
    %ylim([1.5,3]);
    xlabel({'Percentage of compliant individuals';'at the start of the vaccination rollout (\%)'},'interpreter','latex');
...
%yl1=ylabel(han1,{'Cumulative new infections';'since the start of the';'vaccination rollout (\%)'},'interpreter','latex');
