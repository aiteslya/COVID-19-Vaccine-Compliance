...
InfectProp=linspace(0.001,0.01,meshsize);

%calculate the results for the no-vaccination scenario
...
for upsilon=upsilonarr
    ...
    h1(1)=plot(100*InfectProp,Cum3,'LineWidth',4);hold on;
    h1(2)=plot(100*InfectProp,Cum6,'-.','LineWidth',4);hold on;
    xlim([min(100*InfectProp),max(100*InfectProp)]);
    ylim([1,3.5]);
    xlabel({'Percentage of infectious individuals';'at the start of the vaccination rollout (\%)'},'interpreter','latex');
    ...
    h2(1)=plot(100*InfectProp,100*(Cum3-Cum03)./Cum03,'LineWidth',4);hold on;
    h2(2)=plot(100*InfectProp,100*(Cum6-Cum06)./Cum06,'-.','LineWidth',4);hold on;
    xlim([min(100*InfectProp),max(100*InfectProp)]);
    %ylim([1,3.5]);
    xlabel({'Percentage of infectious individuals';'at the start of the vaccination rollout (\%)'},'interpreter','latex');
